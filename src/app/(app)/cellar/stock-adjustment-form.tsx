    ...
    <>
      <div className="mt-sm grid grid-cols-2 gap-xs">
        <label className="text-[12px] text-grey">
          Kind
          <select name="kind" value={form.kind} onChange={(event) => form.setKind(event.target.value as typeof form.kind)} className="mt-xs h-11 w-full rounded-pill border border-hairline bg-white px-sm text-[13px] text-ink">
        ...
        <label className="text-[12px] text-grey">
          Unit
          <select name="unit" value={form.unit} onChange={(event) => form.setUnit(event.target.value as typeof form.unit)} className="mt-xs h-11 w-full rounded-pill border border-hairline bg-white px-sm text-[13px] text-ink">
      ...
      <label className="mt-sm block text-[12px] text-grey">
        Quantity
        <input name="quantity" type="number" step="1" value={form.quantity} onChange={(event) => form.setQuantity(event.target.value)} className="mt-xs h-11 w-full rounded-pill border border-hairline bg-white px-sm font-mono text-[13px] text-ink" />
      </label>
      <label className="mt-sm block text-[12px] text-grey">
        Reason
        <select name="reason_code_id" value={form.reasonId} onChange={(event) => form.setReasonId(event.target.value)} className="mt-xs h-11 w-full rounded-pill border border-hairline bg-white px-sm text-[13px] text-ink">
      ...
      <label className="mt-sm block text-[12px] text-grey">
        Note
        <input name="note" maxLength={500} value={form.note} onChange={(event) => form.setNote(event.target.value)} className="mt-xs h-11 w-full rounded-pill border border-hairline bg-white px-sm text-[13px] text-ink" />
      </label>
      {form.message && <p role="status" className="mt-sm text-[12px] text-grey">{form.message}</p>}
      <button type="button" disabled={form.busy || form.invalid} onClick={form.submit} className="mt-sm h-11 w-full rounded-pill border border-ink/25 bg-white text-[13px] font-medium text-ink hover:bg-bridge-surface disabled:opacity-50">
