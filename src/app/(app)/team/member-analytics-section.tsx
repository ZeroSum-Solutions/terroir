...
export function MemberAnalyticsSection() {
  ...
  if (error) return <p role="alert" className="mt-lg text-[13px] text-danger">{error}</p>;
  if (!data) return <div className="mt-lg h-32 animate-pulse rounded-md bg-surface-muted" />;
  return <MemberAnalyticsTable data={data} />;
}

export function MemberAnalyticsTable({ data }: { data: MemberAnalyticsResult }) {
  return (
    <section aria-labelledby="member-analytics-heading" className="mt-xl rounded-md border border-border bg-white p-md">
      <div className="mb-md flex flex-wrap items-baseline justify-between gap-xs">
        ...
        <span className="text-[12px] text-ink-muted">House median {formatRate(data.houseMedianCompRate)}</span>
      </div>
        ...
        <table className="w-full min-w-[680px] text-left text-[12px]">
          ...
          <tbody>
            {data.members.map((member) => {
              ...
              return (
                <tr id={`member-${member.userId}`} key={member.memberId} className="border-t border-border align-top">
...
function Metric({ href, name, children }: { href: string; name: string; children: React.ReactNode }) {
  return <td data-metric={name} className="py-sm pr-md"><Link href={href} className="text-ink underline decoration-border-strong underline-offset-2">{children}</Link></td>;
}
