...
export async function sendMagicLink(formData: FormData) {
  ...
  // Deliberately omit the email address from the URL and response. Besides
  // reducing referrer exposure, this keeps the response non-enumerating.
...
export async function signInWithPassword(formData: FormData) {
  const next = getNext(formData);
  const email = getEmail(formData);
  if (!email) redirect(loginUrl({ next, password: true, error: "invalid_email" }));
  ...
  if (requestFailed) redirect(loginUrl({ next, password: true, error: "unavailable" }));
...
export async function signUpWithPassword(formData: FormData) {
  const next = getNext(formData);
  const email = getEmail(formData);
  if (!email) redirect(loginUrl({ next, signup: true, error: "invalid_email" }));
  ...
  if (requestFailed) redirect(loginUrl({ next, signup: true, error: "unavailable" }));

  // Supabase intentionally obscures an existing account from signUp. Keep
  // that property at this boundary by using the same response for every
...
export async function sendPasswordReset(formData: FormData) {
  ...
  try {
    ...
    await supabase.auth.resetPasswordForEmail(email, {
      // This supports the standard Supabase template. The deployment contract
      // also installs a token-hash recovery template handled by /auth/confirm.
      redirectTo: authCallbackUrl("/auth/reset-password"),
    });
  } catch {
    // Always return the generic completion state. This endpoint must not
    // disclose whether the email exists or whether the provider rate-limited it.
  }
