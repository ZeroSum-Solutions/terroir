...
export type EnrichSingleWineResult = {
  status: number;
  body: Record<string, unknown>;
};

export async function enrichSingleWine({
  supabase,
  restaurantId,
  wineId,
  signal,
...
}): Promise<EnrichSingleWineResult> {
  ...
  if (
    ...
    const inferred = await enrichWineWithClaude(
      ...
      { signal, throwOnFailure: throwOnProviderFailure },
    );
