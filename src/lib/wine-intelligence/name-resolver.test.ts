...
import pgOracle from "./fixtures/pg-oracle.json";
...
describe("pg_trgm parity (golden vectors from the Postgres-validated impl)", () => {
  ...
  it(`matches the committed live-Postgres oracle on all ${pgOracle.pairs.length} pairs (float4 tolerance)`, () => {
    // pg-oracle.json is generated by fixtures/generate.py --pg against a live
    // Postgres+pg_trgm (version recorded in its provenance block) over the
    // norm()-folded pair forms — the Postgres side of the parity claim as a
    // reproducible committed artifact.
    for (const p of pgOracle.pairs) {
      expect(Math.abs(similarity(p.a, p.b) - p.pg), `pg similarity(${JSON.stringify(p.a)}, ${JSON.stringify(p.b)})`).toBeLessThan(1e-6);
    }
  });
...
describe("audit counterexamples (2026-08-25 GPT-5.6 Sol) — each was a wrong resolution under v3", () => {
  ...
  it("an over-length transcript fails closed (abstain), never truncate-and-score", () => {
    ...
    expect(r.kind).toBe("abstain");
    if (r.kind === "abstain") expect(r.reason).toBe("transcript_too_long");

    // The re-verification audit's probe: a transcript whose contradicting
    // "from Caymus" sits beyond a silent truncation window must NOT resolve.
    const pad = Array.from({ length: 121 }, () => "uh").join(" ");
    const probe = resolveWineName(`Cabernet Sauvignon ${pad} from Caymus`, [
      { itemId: "A", displayName: "Cabernet Sauvignon", producer: "Reserve" },
    ]);
    expect(probe.kind).toBe("abstain");
  });

  it("rejects an oversized candidate list as a caller error", () => {
    const huge = Array.from({ length: 5001 }, (_, i) => ({
      itemId: `X${i}`,
      displayName: `Wine ${i}`,
      producer: `Prod${i}`,
    }));
    expect(() => resolveWineName("musigny", huge)).toThrow(RangeError);
  });
