...
/**
 * Returns the deployment's configured public origin.
 *
 * Authentication email URLs must never be derived from Host or forwarded-host
 * headers: those headers are request input, while Supabase redirect URLs are
 * an environment-level allow-list. Local/test keeps an explicit localhost
 * default; production must configure an HTTPS origin.
 */
export function getAppOrigin(): string {
  ...
  if (
    ...
    throw new Error("NEXT_PUBLIC_APP_URL must be an origin without a path or credentials.");
  }
