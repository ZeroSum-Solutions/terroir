...
export function getRealAuthE2eConfig(
  ...
  const base = parseHttpsUrl(
    required(env, "AUTH_E2E_BASE_URL"),
    "AUTH_E2E_BASE_URL",
  );
  ...
  if (supabase.href.includes(productionPattern)) {
    throw new Error(
      "AUTH_E2E_SUPABASE_URL matches the production safety pattern.",
    );
  }

  const emailDomain = required(env, "AUTH_E2E_EMAIL_DOMAIN").toLowerCase();
  if (
    !/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9-]+)+$/.test(
      emailDomain,
    )
  ) {
    throw new Error(
      "AUTH_E2E_EMAIL_DOMAIN must be a domain used only by the test inbox.",
    );
  }
  ...
  if (!/^[a-z0-9-]{8,80}$/i.test(runId)) {
    throw new Error(
      "AUTH_E2E_RUN_ID must be an opaque 8-80 character run identifier.",
    );
  }
...
export async function waitForMailpitEmail(
  ...
  for (let attempt = 0; attempt < 45; attempt += 1) {
    ...
    if (response.status !== 404) {
      throw new Error(
        `Mailpit returned ${response.status} while waiting for ${recipient}.`,
      );
    }
...
export function extractAuthEmailLink(html: string): string {
  ...
  if (!link) {
    throw new Error("Test mailbox email did not contain an auth token link.");
  }
  return link;
}
