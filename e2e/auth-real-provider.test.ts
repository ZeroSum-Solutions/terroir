...
test.describe("real Supabase email authentication", () => {
  ...
  test("signup, password login, reset, and magic link use only a generated fixture", async ({
    ...
    try {
      ...
      await expect(
        page.getByText("Check your inbox to continue creating your account."),
      ).toBeVisible();
      ...
      await expect(page).toHaveURL(
        new RegExp(`^${escapeForRegExp(config.baseUrl)}`),
      );
...
async function deleteFixtureUser(
  ...
  for (let page = 1; page <= 10; page += 1) {
    ...
    if (error) {
      throw new Error(`Unable to list the isolated auth fixture: ${error.message}`);
    }
    ...
    if (deleteError) {
      throw new Error(
        `Unable to delete the isolated auth fixture: ${deleteError.message}`,
      );
    }
    return;
  }
