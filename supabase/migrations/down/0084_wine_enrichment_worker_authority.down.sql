-- Reverse TER-021G worker authority while retaining the TER-026 web security
-- correction and the tenant-bound LWIN batch contract.
...
revoke execute on function public.match_lwin(text, text, float)
  from service_role;
...
revoke execute on function public.enrich_wines_batch(uuid, jsonb)
  from service_role;
revoke all on function public.match_lwin(text, text, float) from public, anon;
grant execute on function public.match_lwin(text, text, float) to authenticated;
revoke all on function public.match_lwin_batch(uuid, uuid[]) from public, anon;
grant execute on function public.match_lwin_batch(uuid, uuid[]) to authenticated;
revoke all on function public.enrich_wines_batch(uuid, jsonb) from public, anon;
