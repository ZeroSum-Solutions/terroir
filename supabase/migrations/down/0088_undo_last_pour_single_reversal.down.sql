-- Reverse of 0088_undo_last_pour_single_reversal.sql
-- Drops the capacity invariant trigger and restores undo_last_pour to
-- its exact pre-fix (0040) body (reintroduces the double-reversal bug).

drop trigger if exists open_bottles_enforce_capacity_trigger on public.open_bottles;
drop function if exists public.open_bottles_enforce_capacity();
