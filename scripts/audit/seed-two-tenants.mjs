...
async function ensureListItem(sectionId, wineId, attrs = {}) {
  const { data: existing, error: findErr } = await admin
  ...
  const { data: inserted, error: insertErr } = await admin
    .from("wine_list_items")
    .insert({ section_id: sectionId, wine_id: wineId, ...attrs })
    .select("id")
    .single();
...
async function main() {
  ...
  const itemA = await ensureListItem(sectionA, wineA2_published, {
    position: 0,
    glass_price: 14.0,
    bottle_price: 56.0,
    is_available: true,
