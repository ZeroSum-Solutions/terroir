...
**Logged-in walkthrough (2026-09-02, read-only, brief §16)** confirmed the route map and the digest panel set and added four things to adopt: the "Recently Added" holding location plus an invoice-placement modal as one receiving contract (S); per-item depletion order across locations set by drag (S); Sold-As mapping with depletion sizes driving fractional running counts, with per-type serving-size defaults for the POS matcher (M); and an explicit *uncounted* state in variance, because Bevrly's live page computes −100 % variance against zero when nothing has been counted yet. The permission-key claim in row 8 is downgraded: the shipped UI exposes three controls (Inventory Close, Audit Mode, View Costs).
