...
1. **GlobalWineScore appears defunct.** `globalwinescore.com` (incl. `account/api/` and
   ...
   the vendor's API contact mailbox (recorded in the local handoff note; it may outlive the site). The synthesis (v4, frozen
...
1. ✅ **Gmail app password** regenerated for the Workspace mailbox → vault
   `gmail_app_password`. IMAP re-verified working. (Account detail: local handoff note.)
2. ✅ **AssemblyAI** account + key → `assemblyai_api_key`. HTTP 200.
3. ✅ **Deepgram** account (via Google, spike account) + key → `deepgram_api_key`.
   HTTP 200, $200 credit confirmed in console.
4. ✅ **Brave** plan checked — zero monthly allocation (see table); nothing to store.
5. ⏳ **Wine-Searcher** trial application submitted (applicant + account in the local
   handoff note; use case: product matching, market pricing, critic scores,
...
**Account routing** follows the standing local preference recorded in the handoff
note and the fleet memory — not documented here (public repo).
...
4. **PRD approval** — with spikes 1, 4, 5, 6, 7a, 9, 11 closed and `docs/evals/
   vwp-evals.yaml` authored + audit-remediated (wait-list item 4), the ticket-freeze critical path is now:
   PRD sign-off + Gate-0 thresholds (VWP-D-01) + the two externals above.

*(The historical account-automation narrative was moved to the local handoff note — it does not belong in a public repo.)*
...
5. ~~Eval YAML (wait-list item 4)~~ ✅ authored: `docs/evals/vwp-evals.yaml` — 93
   EV-VWP evals across all 12 ★ slices + 8 supporting sections, 13/13 PRD criteria mapped.
6. Voice resolver precursor library landed (`src/lib/wine-intelligence/name-resolver.ts`)
   with the spike-9 replay as its TUNING harness (SPEC-21's sealed partner-weighted holdout is the acceptance gate) — SPEC-20 tickets start from
