...
- `identity_status = 'lwin_verified'` (`'operator_confirmed'` remains unreachable — no promotion RPC exists) is reachable through a raw client insert on `canonical_wines` **or** through `resolve_wine_variants_bulk`, and both paths are gated by the same requirement, enforced identically in both places: the claimed `lwin7` must name a real `lwin_catalog` row whose producer/display name **deterministically** match the submitted producer/cuvée after normalization (exact equality on the normalized text — not a similarity score). A separate table-level `CHECK` constraint additionally makes `lwin7` non-null **only** when `identity_status = 'lwin_verified'`, universally, including for the one-time backfill migration, which runs as the table owner and bypasses RLS entirely.
  ...
  **Round 6 — a THIRD instance of the same class, and a correction to the paragraph above.** Rounds 4 and 5 both attacked the corroboration *gate*. The gate was never the whole exposure: it corroborated `producer`/`cuvee`, while the row's identity key — the columns `canonical_wines_identity_idx` is UNIQUE on, and the columns phase-1 text matching joins on — was `producer_norm`/`cuvee_norm`, a **different pair of caller-supplied fields with nothing anywhere binding one pair to the other**. An attacker therefore never had to defeat the gate at all: submit raws for a wine you legitimately own, whose `lwin7` genuinely corroborates, together with norms naming the victim's wine. Reproduced live end to end before the fix — a row reading `producer = 'Attacker Real Estate'` was stored with `producer_norm = 'estate real victim'`, and the victim's own entirely correct import then bound to it (`canonical_match_method = 'exact'`, `canonical_created = false`), permanently, since the unique index means they can never create their own row and this table grants `authenticated` no UPDATE or DELETE.

  The fix is structural rather than another check: `producer_norm`/`cuvee_norm` are now `GENERATED ALWAYS ... STORED` from `producer`/`cuvee`. A CHECK constraint would have left the caller supplying the key and merely policed it; generation removes the field from every write API, so the decoupling is not defended against — it is unrepresentable, at the database, for every role including `service_role` and the table owner (SQLSTATE `428C9`). This is also why the insert policy still corroborates `producer`/`cuvee` and needs no `producer_norm` counterpart: those two columns are now provably the sole inputs to the key, so corroborating them *is* corroborating it.

  **The sentence above about residual risk was wrong from round 5 onward, and is corrected here rather than quietly rewritten.** It claims a SQL-vs-TypeScript normalization mismatch can only cause a false negative. That held while the SQL side merely *compared*. Round 5 moved identity-key derivation server-side, at which point the same mismatch became capable of a false **positive** — and immediately was one: `identity_normalize_text` had no counterpart to `normalize.ts`'s possessive-suffix rule, so `"O'Brien's Vineyard"` and `"O.S. Brien Vineyard"` — the exact D3 pair §5's round-2 fix separated — both normalized to `brien o s vineyard` and would have shared one canonical identity. Measured against the frozen golden vectors: 10 of 17 agreed before the rule was ported into SQL, 17 of 17 after, every failure that one cause. The guarantee is now upheld by enforcement rather than by argument — `src/domains/identity/normalize.test.ts` asserts the live database function against the frozen contract, and `supabase/tests/0099_resolve_wine_variants_bulk.sql`'s D3 case drives the real RPC so the rule is also pinned by a test that always runs under `supabase test db`. `identity_normalize_text` additionally pins its `search_path`, since an IMMUTABLE marking over a STABLE `unaccent()` is a survivable promise for a comparison and not one for a stored, indexed identity key.
