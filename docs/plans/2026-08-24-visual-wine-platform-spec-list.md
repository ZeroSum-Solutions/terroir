...
| 0129+ | reserved | Durable import staging IF VWP-D-04 chooses it; `wine_edition_formats` when barcode authority needs it; splat asset metadata if the spike passes | per decision | — |
...
0127–0128 are allocated from the reserved band and may land before that sequence; on a
...
- **SPEC-07 Import scale-up (20k CSV)** — VWP-FR-008. Durable staging vs. `MAX_ROWS`
  raise decided by VWP-D-04 (staging → takes an 0129+ number). Acceptance: full partner
...
- **SPEC-14 Splat overlay** — desktop-only, ships only on spike 2 pass (VWP-D-06);
  Polycam→GaussianSplats3D; asset metadata migration only if approved (0129+).
