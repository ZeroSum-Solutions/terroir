...
| D16 | The claret-derived rating ramp is **cut too**. A score renders in ordinary ink with its provenance sentence beside it | Amended 2026-09-03. D10 kept this ramp as the one colour concession; building it showed the contract forbids it for the same reasons it forbids the family palette. A rating→warmth scale is an information hue, and DESIGN.md says there must not be one because "a fifth colour would dilute the thesis to buy nothing". The thesis is that "photography carries the warmth so no surface has to" — tinting a numeral puts warmth on a surface. And the Status section already refuses this exact trade: "At peak" is achromatic *because* the good, quiet, common state "should not compete for attention with the two states that need action" — so giving a high score the one claret makes a 98-point wine compete with Window risk, in the same hue, with the valence inverted. The numeral and its basis sentence already carry the whole claim. §5.6 stands as the record of what was measured. |
| D10 | No per-aroma-family colour, and (per D16) no rating ramp either | See §5.6 — the family palette is the exact zone `check-design-palette.mjs` bans, and DESIGN.md forbids a fifth hue. This is a documented departure from the adoption plan's §2.1 item 4. |
